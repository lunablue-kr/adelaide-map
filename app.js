...
function glyphHtml(cat,color,d){
  ...
  return `<div class="gpin" style="width:${d}px;height:${d}px;background:${color};--pc:${color}"><svg width="${s}" height="${s}" viewBox="0 0 24 24">${GLYPHS[cat]}</svg></div>`;
}
function poiMarker(ll,o){
  ...
  mk._pc=o.color; // 격리(hollow) 복원용 카테고리색
...
function dimMarker(mk,visible,front){ // 격리: 비선택은 테두리만(hollow), 선택은 솔리드로 맨 앞
  if(mk instanceof L.CircleMarker){
    if(visible)mk.setStyle({fillOpacity:1,opacity:1,color:'#0c0f14',weight:1.1});
    else mk.setStyle({fillOpacity:0,opacity:0.95,color:mk._pc||'#8890a8',weight:1.6}); // 속 비고 색테두리만
    if(visible&&front)mk.bringToFront();
  }else{
    mk.setOpacity(1);
    const el=mk.getElement&&mk.getElement(),g=el&&el.querySelector('.gpin');
    if(g)g.classList.toggle('ghost',!visible);
    else if(!visible)mk.setOpacity(0.22); // 엘리먼트 미렌더 시 폴백
...
function renderMiniLegend(){
  ...
  if(pubOn){
    html+=`<div class="ml-title" style="margin-top:7px"><span class="ml-glyph"><svg width="13" height="13" viewBox="0 0 24 24">${GLYPHS.pub}</svg></span>${t.layers.pubs}</div>`+
      ['pub','bar','wine','brew','club'].map(k=>`<div class="ml-item ml-click${pubFilter&&pubFilter!==k?' dim':''}" data-pub="${k}"><span class="ml-dot" style="background:${PUB_COLOR[k]}"></span>${t.pubTypes[k]}</div>`).join('');
  }
  if(parkOn){
